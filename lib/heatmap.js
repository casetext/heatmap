...
Heatmap.prototype._bindEvents = function() {
  ...
  .on('mouseenter', function() {
    self.emit('sourceMouseenter', d3.mouse(this));
  })
  .on('mouseleave', function() {
    self.emit('sourceMouseleave', d3.mouse(this));
  })
  .on('mousemove', function() {
    self.emit('sourceMousemove', d3.mouse(this));
  })
  .on('click', function() {
    self.emit('sourceClick', d3.mouse(this));
  });

  this.target
  .on('mouseenter', function() {
    self.emit('targetMouseenter', d3.mouse(this));
  })
  .on('mouseleave', function() {
    self.emit('targetMouseleave', d3.mouse(this));
  })
  .on('mousemove', function() {
    self.emit('targetMousemove', d3.mouse(this));
  })
  .on('click', function() {
    self.emit('targetClick', d3.mouse(this));
  });
